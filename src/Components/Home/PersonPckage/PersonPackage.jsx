import React from 'react';

const PersonPackage = () => {
    return (
        <div>
            <h3 className='text-[13px] uppercase font-medium text-yellow-400 text-center'>featured</h3>
            <h1 className='text-xl font-medium my-1 text-center'>Hajj and Umrah Packages</h1>
            <div className='md:flex md:mx-20'>
                <div className='md:w-[100%] md:mx-10 mx-4 border rounded mt-4'>
                    <div className='bg-yellow-500 ps-4 pt-4 pb-9 relative'>
                        <h3 className='font-medium'>Hajj</h3>
                        <p className='text-[13px] text-black'>$8000 USD / Person</p>
                        <button className='uppercase text-[10px] font-medium text-yellow-400 bg-black px-4 py-3 rounded absolute -bottom-5'>get stared</button>
                    </div>
                    <div className='md:space-y-5 space-y-2 pb-5'>
                        <div className='flex items-center ms-4 mt-10'>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-blue-700 border rounded-full p-1">
                                    <path fillRule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clipRule="evenodd" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>Private Airport Transfers</p>
                            </div>
                        </div>
                        <div className='flex items-center ms-4 '>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-red-500 border rounded-full p-1">
                                    <path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>
                                    Umrah Visa</p>
                            </div>
                        </div>
                        <div className='flex items-center ms-4 '>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-red-500 border rounded-full p-1">
                                    <path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>Land Transportation</p>
                            </div>
                        </div>
                        <div className='flex items-center ms-4 '>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-blue-700 border rounded-full p-1">
                                    <path fillRule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clipRule="evenodd" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>
                                    Accodomation</p>
                            </div>
                        </div>
                        <div className='flex items-center ms-4 '>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-blue-700 border rounded-full p-1">
                                    <path fillRule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clipRule="evenodd" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>Lunch</p>
                            </div>
                        </div>
                        <div className='flex items-center ms-4 '>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-red-500 border rounded-full p-1">
                                    <path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>Dinner
                                </p>
                            </div>
                        </div>
                        <div className='flex items-center ms-4 '>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-red-500 border rounded-full p-1">
                                    <path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>Private Airport Transfers</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div className='md:w-[100%] md:mx-10 mx-4 border rounded mt-4'>
                    <div className='bg-black ps-4 pt-4 pb-9 relative'>
                        <h3 className='font-medium text-white'>Umrah</h3>
                        <p className='text-[13px] text-yellow-400'>$8000 USD / Person</p>
                        <button className='uppercase text-[10px] font-medium text-yellow-400 bg-slate-200 px-4 py-3 rounded absolute -bottom-5'>get stared</button>
                    </div>
                    <div className='md:space-y-5 space-y-2 pb-5'>
                        <div className='flex items-center ms-4 mt-10'>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-blue-700 border rounded-full p-1">
                                    <path fillRule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clipRule="evenodd" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>Private Airport Transfers</p>
                            </div>
                        </div>
                        <div className='flex items-center ms-4 '>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-red-500 border rounded-full p-1">
                                    <path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>
                                    Umrah Visa</p>
                            </div>
                        </div>
                        <div className='flex items-center ms-4 '>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-red-500 border rounded-full p-1">
                                    <path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>Land Transportation</p>
                            </div>
                        </div>
                        <div className='flex items-center ms-4 '>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-blue-700 border rounded-full p-1">
                                    <path fillRule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clipRule="evenodd" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>
                                    Accodomation</p>
                            </div>
                        </div>
                        <div className='flex items-center ms-4 '>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-blue-700 border rounded-full p-1">
                                    <path fillRule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clipRule="evenodd" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>Lunch</p>
                            </div>
                        </div>
                        <div className='flex items-center ms-4 '>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-red-500 border rounded-full p-1">
                                    <path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>Dinner
                                </p>
                            </div>
                        </div>
                        <div className='flex items-center ms-4 '>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-red-500 border rounded-full p-1">
                                    <path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>Private Airport Transfers</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div className='md:w-[100%] md:mx-10 mx-4 border rounded mt-4'>
                    <div className='bg-slate-200 ps-4 pt-4 pb-9 relative'>
                        <h3 className='font-medium'>Hajj Plus</h3>
                        <p className='text-[13px]'>$10000 USD / Person</p>
                        <button className='uppercase text-[10px] font-medium bg-yellow-400 text-black px-4 py-3 rounded absolute -bottom-5'>get stared</button>
                    </div>
                    <div className='md:space-y-5 space-y-2 pb-5'>
                        <div className='flex items-center ms-4 mt-10'>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-blue-700 border rounded-full p-1">
                                    <path fillRule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clipRule="evenodd" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>Private Airport Transfers</p>
                            </div>
                        </div>
                        <div className='flex items-center ms-4 '>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-red-500 border rounded-full p-1">
                                    <path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>
                                    Umrah Visa</p>
                            </div>
                        </div>
                        <div className='flex items-center ms-4 '>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-red-500 border rounded-full p-1">
                                    <path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>Land Transportation</p>
                            </div>
                        </div>
                        <div className='flex items-center ms-4 '>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-blue-700 border rounded-full p-1">
                                    <path fillRule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clipRule="evenodd" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>
                                    Accodomation</p>
                            </div>
                        </div>
                        <div className='flex items-center ms-4 '>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-blue-700 border rounded-full p-1">
                                    <path fillRule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clipRule="evenodd" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>Lunch</p>
                            </div>
                        </div>
                        <div className='flex items-center ms-4 '>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-red-500 border rounded-full p-1">
                                    <path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>Dinner
                                </p>
                            </div>
                        </div>
                        <div className='flex items-center ms-4 '>
                            <div className=''>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-6 h-6 text-red-500 border rounded-full p-1">
                                    <path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" />
                                </svg>

                            </div>
                            <div>
                                <p className='text-sm ms-2'>Private Airport Transfers</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    );
};

export default PersonPackage;

